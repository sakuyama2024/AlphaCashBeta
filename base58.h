...
inline std::string EncodeBase58(const unsigned char* pbegin, const unsigned char* pend)
{
    ...
    std::string str;
    str.reserve((pend - pbegin) * 138 / 100 + 1); // log(256) / log(58), rounded up
    CBigNum dv;
    CBigNum rem;
    ...
    while (BN_cmp(bn.bn, bn0.bn) > 0)
    {
        ...
        if (!BN_copy(bn.bn, dv.bn))
            throw bignum_error("EncodeBase58 : BN_copy failed");
        unsigned int c = BN_get_word(rem.bn);
        str += pszBase58[c];
    }
...
inline bool DecodeBase58(const char* psz, vector<unsigned char>& vchRet)
{
    ...
    for (const char* p = psz; *p; p++)
    {
        ...
        if (!BN_add(bn.bn, bn.bn, bnChar.bn))
